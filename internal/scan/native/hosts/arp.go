...
func (p *arpProber) Probe(ctx context.Context, ip netip.Addr, opts scan.HostsOptions) (scan.Result, error) {
	result := scan.Result{
		Addr:   ip,
		Status: scan.StatusUnknown,
		Method: scan.MethodARP,
		Meta:   map[string]any{},
	}

	if !canUseARP(ip) {
		result.Meta["reason"] = "arp_unsupported"
		return result, nil
	}
	...
	//
	// addr := netip.AddrPortFrom(ip, uint16(65534))
	// start := time.Now()
